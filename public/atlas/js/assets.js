...
const Assets = (() => {
  ...
  async function loadIconSet() {
    iconSetSrc = window.RPGATLAS_ICON_SET || assetUrl("img/system/icon_set.png?v=" + Date.now());
  ...
  async function discoverFolder(type) {
    try {
      const res = await fetch(assetUrl("img/" + type + "/"));
      if (!res.ok) return [];
  ...
  async function discoverExternalAssets() {
    ...
    try {
      const res = await fetch(assetUrl("img/assets.json"));
      if (res.ok) {
        const manifest = await res.json();
        const listed = [];
        for (const type of Object.keys(external)) {
          for (const file of manifest[type] || []) {
            listed.push({ type, name: assetName(file), src: assetUrl("img/" + type + "/" + file) });
          }
        }
        return listed;
      }
